...
# response = requests.get(url, proxies=proxies) 代理格式
...
def down_pic():
    ...
    try:
        for item in bs.find_all('a', class_="fileThumb", href=re.compile('data')):  # 找到原画质图片位置
            ...
            print(f"开始下载 {down_name} 图片文件..")  # 输出开始下载文件
            # 检查图片是否存在
            if os.path.exists(down_name):
                count = 0
                while os.path.getsize(down_name) == 11279 and count <= 5:
                    os.remove(down_name)
...
# 下载附件
def down_file():
    global r, item, soup, bs, down_name, down_size, count
    try:
    ...
    for i in range(file_count):
        ...
        if os.path.exists(down_name) or ('psd' or 'PSD' or 'clip') in down_name:
            ...
            try:
                ...
                down_size = r.headers['Content-Length']
                count = 0
            except:
