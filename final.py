...
print('选择多画师下载模式需要在该程序目录下存在"task.txt",且每行只有一个链接\n选择y进入多画师模式，选择n进入普通模式')
...
if que == 'y':
    ...
    for i in range(num):
        ...
        if ('fanbox/' or 'patron' or 'fantia') and 'user/' and 'o=' in str(final_list[i]):  # 判断是否为页面链接
            ...
            num = len(final_list2)
            for i in range(num):
                if ('fanbox/' or 'patron' or 'fantia') and 'user/' and 'post/' in str(href_list2[i]):  # 判断是否为含资源的页面
                    url = 'https://kemono.party' + str(href_list2[i])  # 设置url链接
                    down_url(url)  # 执行下载
                    print('开始下载：'+url)
    print('所有下载任务完成')
