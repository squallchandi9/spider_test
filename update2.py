...
def down_url(url):  # 定义函数
    global bs, item, soup, r, down_size, count, count_f, down_name, down_url_p
    try:
    ...
    for i in range(file_count):
        ...
        if os.path.exists(down_name) or ('psd' or 'PSD' or 'clip') in down_name_gs:
        ...
        for item in bs.find_all('a', class_="fileThumb", href=re.compile('data')):  # 找到原画质图片位置
            ...
            if os.path.exists(down_name):
                ...
                try:
                    ...
                    print('请求失败！')
                while os.path.getsize(down_name) != down_size and count <= 5:
                    os.remove(down_name)
